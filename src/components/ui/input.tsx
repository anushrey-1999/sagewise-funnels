...
function Input({
  ...
  return (
    <input
      ...
      className={cn(
        "file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 border-aw-border w-full min-w-0 rounded-[var(--radius-field)] border h-[58px] min-h-[58px] px-3 py-1 text-base shadow-[var(--field-shadow)] transition-[color,box-shadow,background-color,border-color] duration-200 ease-out will-change-[color,box-shadow,transform,opacity] motion-reduce:transition-none motion-reduce:will-change-auto outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm cursor-pointer hover:border-aw-border-strong hover:shadow-[var(--field-shadow-hover)] focus-visible:[box-shadow:var(--focus-ring)] bg-white!",
        isFocused && !ariaInvalid && "border-sg-primary-green!",
        isFocused && ariaInvalid && "border-destructive!",
        "aria-invalid:border-destructive aria-invalid:focus-visible:[box-shadow:var(--focus-ring-error)]",
