...
// Already brand sage, so it must not go through the wordmark tint filter.
const imgTypeLogomark = "/sagewise-mark.png";
...
export function Logo({ className, color = "primary", type = "wordmark", href = "/", width = 161, height = 35, imgClassName }: LogoProps) {
  const isLogomark = type === "logomark";

  const src = isLogomark
    ? imgTypeLogomark
    : color === "white" && type === "wordmark"
      ? imgColorWhiteTypeWordmark
      : imgColorPrimaryTypeWordmark;

  const tintFilter =
    !isLogomark && color === "primary" ? PRIMARY_WORDMARK_FILTER : undefined;
          ...
          style={
            imgClassName
              ...
              : {
                  width: `${width}px`,
                  height: `${height}px`,
                  ...(tintFilter ? { filter: tintFilter } : null),
                }
          }
        />
      </Link>
